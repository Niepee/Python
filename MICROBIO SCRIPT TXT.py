...
DEST_BTU = "U:\\Python\\Microbio\\BTU"
DEST_BTU_BLOEDKWEEK = "U:\\Python\\Microbio\\BTU\\Bloedkweek"
...
for f in file_list:
    with open(f, 'r') as file:
        for line in file:
            if 'Bloedkweek' in line:
                shutil.copy(f, DEST_BTU_BLOEDKWEEK)
            if 'BTU' in line:
...
# scanning through BTU om overtollige bloedkweek te verwijderen
for (dirpath, dirnames, filenames) in os.walk(DEST_BTU):
    for r in filenames:
        if 'txt' in str(r):
            e = os.path.join(str(dirpath), str(r))
            file_list.append(e)
...
# BTU bestanden lezen
# defining location of parent folder
BASE_DIRECTORY_BTU = "U:\\Python\\Microbio\\BTU"
output_file_BTU = open("U:\\Python\\Microbio\\output_BTU.txt", 'w')
output = {}
file_list = []
...
# scanning through sub folders
for (dirpath, dirnames, filenames) in os.walk(BASE_DIRECTORY_BTU):
    for f in filenames:
        if 'txt' in str(f):
            e = os.path.join(str(dirpath), str(f))
            file_list.append(e)
...
for tab in tabs:
    output_file_BTU.write('\n')
    for row in output[tab]:
        output_file_BTU.write(row + '')
    output_file_BTU.write('\n')
    output_file_BTU.write('---')

output_file_BTU.close()
...
# Extract output_CB naar tabel
# Open the text file and read its contents & split the data into individual strings
f = open("U:\\Python\\Microbio\\output_BTU.txt", 'r')
data = f.read()
strings = re.split('---', data)
strings.remove('')
...
# Create an empty list to store the table rows
table = []
df = pandas.DataFrame(columns=['Plaat Nr.', 'Datum', 'Naam', 'Type', 'Soort', 'Determinatie'])
...
for i, s in enumerate(strings):
    ...
    # Append the row to the table
    new_row = [i+1, date, name, type, soort, Determinatie]
    df.loc[len(df)] = new_row
...
df2.to_excel(excel_writer="U:\\Python\\Microbio\\output_BTU.xlsx", sheet_name= "Datasheet")
