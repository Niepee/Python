...
with open("C:\\Users\\Niels\\Desktop\\Python\\SCRIPTS\\output_CB.txt", 'r') as f:
    data = f.read()
...
table = []
df = pandas.DataFrame(columns=['Nr.', 'Datum', 'Naam', 'Type', 'Locatie', 'Determinatie'])
...
df2=(df.groupby(df.columns.drop('Determinatie').tolist()) #group by all but one column
...
df2['Determinatie'].str.strip()

df2.to_csv(path_or_buf="C:\\Users\\Niels\\Desktop\\Python\\SCRIPTS\\output_CB.csv")
df2.to_excel(excel_writer="C:\\Users\\Niels\\Desktop\\Python\\SCRIPTS\\output_CB.xlsx")
